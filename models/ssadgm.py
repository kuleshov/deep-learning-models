...
class SSADGM(SemiSupModel):
  ...
  def __init__(self, X_labeled, y_labeled, n_out, n_superbatch=12800, model='bernoulli',
    ...
    self.model = model
    self.n_out = n_out
    self.n_sample = 1 # monte-carlo samples; need to make this a command-line param
  ...
  def create_model(self, L, yl, n_dim, n_out, n_chan=1):
    ...
    # save this for later (hack; should be save elsewhere)
    self.n_out = n_out
    ...
    if self.model == 'bernoulli':
      l_px_mu = lasagne.layers.DenseLayer(l_px_hid3, num_units=n_vis,
          nonlinearity = lasagne.nonlinearities.sigmoid,
          W=lasagne.init.GlorotUniform(),
          b=lasagne.init.Normal(1e-3))
    elif self.model == 'gaussian':
      l_px_mu = lasagne.layers.DenseLayer(
          l_px_hid3, num_units=n_vis,
          nonlinearity=None)
  ...
  def create_objectives(self, X, L, yu, yl, deterministic=False):
    ...
    y_prior        = T.cast(T.ones((n_lab*n_sam, n_out)) / n_out, dtype=theano.config.floatX)
    ...
    elbo = T.mean(log_paxzy - log_qza_given_xy)
